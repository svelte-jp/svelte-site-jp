...
> サーバーサイドレンダリング（SSR）のために、この`fetch`は、`<script>` の最上位ではなく、 `onMount` の中に入れることが推奨されます。SSRの間には、`onDestroy`以外のライフサイクル関数が動作することはなく、このコンポーネントがDOMにレンダリングされた上で遅延して読み込まれるべきデータを取得せずに済みます。
