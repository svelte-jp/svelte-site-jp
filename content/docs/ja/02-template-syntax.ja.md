...
each ブロックは `array.map(...)` のコールバックの第 2 引数に相当するインデックスを指定することもできます。
...
key の式（各リストアイテムを一意に識別できる必要があります）が与えられた場合、Svelte は末尾にアイテムを追加したり削除するのではなく、データが変更されたときに差分をとるために使用します。key はどんなものでもよいですが、オブジェクト自体が変更されたときに同一性を維持できるため、文字列か数値をお勧めします。
